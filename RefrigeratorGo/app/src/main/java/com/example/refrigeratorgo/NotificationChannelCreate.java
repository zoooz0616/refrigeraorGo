...
//            ArrayList<byte[]> foodNameImage = get.getStringArrayListExtra("foodImage"); //사진배열 받아
    ...
    private void nofi_day() {
        int todayNum = Integer.valueOf(doCurrentDate());
//        int dDateNum = Integer.valueOf(dDate);


        for (int i = 0; i < arrayDataArrayList.size(); i++) {
            Log.i("alarmDataArrayList: ", "gg");
            int dDateNum = Integer.valueOf(arrayDataArrayList.get(i).getFoodDate());
